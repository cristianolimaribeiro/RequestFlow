  ...
  useEffect(() => {
    async function loadRequests() {
      try {
        ...
        setError(err.message || 'Erro ao carregar todas as solicitações');
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="page-container">
      <header className="page-header">
        ...
        <button 
          onClick={() => exportRequestsToPDF(requests, "Todas as Solicitações - RequestFlow")} 
      ...
      {requests.length === 0 ? (
        <EmptyState message="Nenhuma solicitação encontrada no sistema." />
      ) : (
